...
class reportError(bpy.types.Operator):
    ...
    bl_idname = "scene.legoizer_report_error"                                            # unique identifier for buttons and menu items to reference.
    ...
    def execute(self, context):
        ...
        writeErrorToFile(libraryServersPath, 'LEGOizer_log', props.addonVersion)
        # open error report in UI with text editor
        changeContext(context, "TEXT_EDITOR")
        try:
            bpy.ops.text.open(filepath=os.path.join(libraryServersPath, "LEGOizer_error_report.txt"))
            bpy.context.space_data.show_word_wrap = True
            self.report({"INFO"}, "Opened 'LEGOizer_error_report.txt'")
            bpy.props.needsUpdating = True
        except:
            self.report({"ERROR"}, "ERROR: Could not open 'LEGOizer_error_report.txt'. If the problem persists, try reinstalling the add-on.")
        return{"FINISHED"}
...
class closeReportError(bpy.types.Operator):
    ...
    bl_idname = "scene.legoizer_close_report_error"                                            # unique identifier for buttons and menu items to reference.
    bl_label = "Close Report Error"                                                   # display name in the interface.
    bl_options = {"REGISTER", "UNDO"}

    def execute(self, context):
        txt = bpy.data.texts['LEGOizer_log']
