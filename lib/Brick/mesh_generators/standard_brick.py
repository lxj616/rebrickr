...
def makeStandardBrick(dimensions:dict, brickSize:list, brickType:str, circleVerts:int=16, detail:str="Low Detail", logo:Object=None, stud:bool=True, bme:bmesh=None):
    ...
    # set dimension and thickness variables
    ...
    if stud:
        ...
        for xNum in range(brickSize[0]):
            for yNum in range(brickSize[1]):
                ...
                _,cylVerts = makeCylinder(r=dimensions["stud_radius"], h=dimensions["stud_height"]+studInset, N=circleVerts, co=Vector((xNum*d.x*2,yNum*d.y*2,zCO)), botFace=False, bme=bme)
                selectVerts(cylVerts["bottom"])
    ...
    if detail != "Flat":
        ...
        if detail == "High Detail" and ((brickSize[0] == 2 and brickSize[1] > 1) or (brickSize[1] == 2 and brickSize[0] > 1)) and brickSize[2] != 1:
            addTickMarks(dimensions, brickSize, circleVerts, detail, d, thickXY, v1, v3, v5, v7, v9, v11, v13, v15, bme)
        else:
        ...
        # make tubes
        addTubeSupports(brickSize, dimensions, brickType, circleVerts, detail, d, thickXY, bme)
        # Adding bar inside 1 by x bricks
        addBars(dimensions, brickSize, circleVerts, detail, d, thickXY, z1, z2, bme)
        # add small inner cylinders inside brick
        if detail in ["Medium Detail", "High Detail"]:
            addInnerCylinders(dimensions, brickSize, circleVerts, d, v10, v12, v14, v16, bme)
...
def addTickMarks(dimensions, brickSize, circleVerts, detail, d, thickXY, v1, v3, v5, v7, v9, v11, v13, v15, bme):
    thickZ = dimensions["thickness"]
...
def addTubeSupports(brickSize, dimensions, brickType, circleVerts, detail, d, thickXY, bme):
    thickZ = dimensions["thickness"]
    ...
    if brickType in ["Bricks and Plates"] and brickSize[2] == 3:
        z1 = d.z-thickZ-dimensions["support_height_triple"]
    else:
        z1 = d.z-thickZ-dimensions["support_height"]
    z2 = d.z-thickZ
...
def addBars(dimensions, brickSize, circleVerts, detail, d, thickXY, z1, z2, bme):
    thickZ = dimensions["thickness"]
    if brickSize[0] == 1:
...
def addInnerCylinders(dimensions, brickSize, circleVerts, d, v10, v12, v14, v16, bme):
    thickZ = dimensions["thickness"]
